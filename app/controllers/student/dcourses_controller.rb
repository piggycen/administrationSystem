# class Student::DcoursesController < ApplicationController
# 	def index
# 		params.permit!
# 		@user = User.find_by(no: params[:user])
# 		@dcourseall = Course.where(:ctype => "D")
# 	end
# 	def new
# 		params.permit!
# 		@user = User.find_by(no: params[:user])
...
# 		@dcourse = Course.find(params[:courseid])
...
# 		if @mycourse.save
# 			redirect_to student_dcourses_path(:user => @user.no)
# 		else
# 			render 'new'
# 		end
# 	end
...
class Student::DcoursesController < ApplicationController
	def index
		params.permit!
		@user = User.find_by(no: params[:user])
		@acourseall = Course.where(:ctype => "D")
	end
	def new
		params.permit!
		@user = User.find_by(no: params[:user])
