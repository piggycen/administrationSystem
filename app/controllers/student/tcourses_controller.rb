# class Student::ScoursesController < ApplicationController
# 	def index
# 		params.permit!
# 		@user = User.find_by(no: params[:user])
# 		@scourseall = Course.where(:ctype => "S")
# 	end
# 	def new
# 		params.permit!
# 		@user = User.find_by(no: params[:user])
...
# 		@scourse = Course.find(params[:courseid])
...
# 		if @mycourse.save
# 			redirect_to student_scourses_path(:user => @user.no)
# 		else
# 			render 'new'
# 		end
# 	end
...
class Student::BcoursesController < ApplicationController
	def index
		params.permit!
		@user = User.find_by(no: params[:user])
		@acourseall = Course.where(:ctype => "T")
	end
	def new
		params.permit!
		@user = User.find_by(no: params[:user])
