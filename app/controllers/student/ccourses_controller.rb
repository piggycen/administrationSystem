# class Student::CcoursesController < ApplicationController
# 	def index
# 		params.permit!
# 		@user = User.find_by(no: params[:user])
# 		@ccourseall = Course.where(:ctype => "C")
# 	end
# 	def new
# 		params.permit!
# 		@user = User.find_by(no: params[:user])
...
# 		@ccourse = Course.find(params[:courseid])
...
# 		if @mycourse.save
# 			redirect_to student_ccourses_path(:user => @user.no)
# 		else
# 			render 'new'
# 		end
# 	end
...
class Student::CcoursesController < ApplicationController
	def index
		params.permit!
		@user = User.find_by(no: params[:user])
		@acourseall = Course.where(:ctype => "C")
	end
	def new
		params.permit!
		@user = User.find_by(no: params[:user])
